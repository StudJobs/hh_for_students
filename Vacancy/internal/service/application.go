...
type Application interface {
	...
	Get(ctx context.Context, id string) (*applicationv1.Application, error)
...
func (s *ApplicationService) Get(ctx context.Context, id string) (*applicationv1.Application, error) {
	if id == "" {
		return nil, ErrInvalidApplicationData
	}
...
func (s *ApplicationService) AssignHR(ctx context.Context, id, hrUserID string) (*applicationv1.Application, error) {
	if id == "" || hrUserID == "" {
		return nil, ErrInvalidApplicationData
	}
	return s.repo.AssignHR(ctx, id, hrUserID)
}
