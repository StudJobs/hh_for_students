...
func (r *UsersRepository) CreateProfile(ctx context.Context, profile *usersv1.Profile) (*usersv1.Profile, error) {
	...
	if len(profile.SkillSlugs) > 0 {
		insertBuilder = insertBuilder.Columns("skill_slugs")
		values = append(values, profile.SkillSlugs)
	}

	if profile.Github != "" {
		insertBuilder = insertBuilder.Columns("github")
		values = append(values, profile.Github)
	}
...
func (r *UsersRepository) UpdateProfile(ctx context.Context, id string, profile *usersv1.Profile) (*usersv1.Profile, error) {
	...
	if len(profile.SkillSlugs) > 0 {
		updateBuilder = updateBuilder.Set("skill_slugs", profile.SkillSlugs)
	}
	if profile.Github != "" {
...
func (r *UsersRepository) AddVerifiedSkills(ctx context.Context, userID string, slugs []string) (*usersv1.Profile, error) {
	if userID == "" || len(slugs) == 0 {
		return r.GetProfile(ctx, userID)
	}
	// PostgreSQL: добавляем каждый slug, если его ещё нет в массиве.
	// `array_cat` + `DISTINCT` через подзапрос: SELECT array_agg(DISTINCT x) FROM unnest(...).
	query := `
UPDATE profiles
SET verified_skill_slugs = (
		SELECT ARRAY(SELECT DISTINCT unnest(verified_skill_slugs || $2::varchar[]))
	),
	updated_at = NOW()
WHERE id = $1 AND deleted_at IS NULL`
	_, err := r.db.Exec(ctx, query, userID, slugs)
	if err != nil {
		log.Printf("Repository: AddVerifiedSkills failed user=%s: %v", userID, err)
